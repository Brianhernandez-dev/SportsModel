# NFL Historical Market Research Preregistration 0.2.5
...
`nfl_historical_market_research_0.2.4`. The complete `0.1.0`, `0.2.0`, `0.2.1`,
`0.2.2`, `0.2.3`, and `0.2.4` preregistrations remain immutable historical
records in
...
and
[NFL Historical Market Research Preregistration 0.2.4](nfl_historical_market_research_protocol_0.2.4.md).

These amendments were specified before any historical odds were joined to model
outputs, outcomes, or derived betting performance. Version `0.2.5` narrowly
closes consensus reference-market coverage semantics, locked-check terminal-state
precedence, and exact RNG call sequencing. It does not redesign `0.2.4`. All
...
For a game to have an eligible reference market, the frozen designated reference
book must contribute both eligible full-game moneyline sides in one observation
that satisfies the authoritative timestamp, `T-60`, `maximum_snapshot_age`, and
all other frozen market rules. If the designated book has no such valid reference
observation, exclude the game from counted-wager eligibility and emit
`REFERENCE_MARKET_UNAVAILABLE`. Do not substitute another sportsbook.

### B. `EQUAL_WEIGHT_NO_VIG_CONSENSUS`

The final provider amendment must freeze an eligible reference-book universe of
at least two sportsbooks before any joined-performance computation. For every
sportsbook in that frozen universe, independently calculate the selected-side
proportional two-way no-vig probability under the `Decimal` contract. Calculate
the arithmetic mean of all resulting book probabilities with exactly equal
weight. Do not quantize intermediate values.
...
at least two sportsbooks before any joined-performance computation. For every
sportsbook in that frozen universe, independently calculate the selected-side
proportional two-way no-vig probability under the `Decimal` contract. Order those
`Decimal` probabilities ascending. For an odd book count, use the middle value.
For an even book count, use the arithmetic mean of the two central values under
the existing `Decimal` context. Do not quantize intermediate values.

For either consensus method, every sportsbook in the frozen reference-book
universe must contribute a valid eligible two-sided `T-60` full-game moneyline
observation for each historical game under the frozen timestamp and
`maximum_snapshot_age` rules. If any frozen reference book lacks such an
observation, the reference market is incomplete: exclude the game before a
counted wager exists and emit `REFERENCE_MARKET_INCOMPLETE`.

For an incomplete consensus reference market, do not shrink the reference-book
universe for that game, compute consensus from an available subset, substitute a
different book, search a different timestamp outside the frozen selector, change
the minimum coverage after performance exposure, or impute a missing quote.
Consensus membership therefore cannot vary game by game.
...
licensing, and reproducibility. Outcome-blind provider feasibility may use
historical coverage and continuity evidence when choosing the frozen universe.
Historical model profitability, historical wagering ROI, result-dependent
weighting, discretionary `sharp book` weighting, provider or book weights
optimized from joined performance, and any fourth reference-market construction
invented after performance exposure are prohibited.
...
Initialize exactly one fresh generator for the complete 2021-2023 development
bootstrap using exactly:
...
The ordered season-specific common-cluster frames are 2021, 2022, and 2023, in
that order, with each frame already ordered by the protocol's canonical
cluster-ordering rule. For bootstrap draw indices `b = 0, 1, 2, ..., 9999`,
process seasons in exactly 2021, then 2022, then 2023 order. For each season `s`,
with `n_s` observed clusters, make exactly one RNG call:
...
Interpret the returned integers as zero-based indices into that season's
canonical ordered cluster frame. Each development draw therefore performs
exactly three RNG calls in fixed 2021 to 2022 to 2023 order. All eight thresholds
use the exact same sampled cluster multiset for each draw.

Do not call `choice`, batch seasons in a different order, generate all seasons in
a different vectorized call, discard RNG output, redraw based on wager counts, or
consume RNG state for diagnostics between required calls.
...
- seed `20260912`;
- NumPy version;
- each canonical ordered season-specific cluster frame;
- the complete generated index evidence or an immutable canonical representation
  of the draws; and
...
single frozen policy in each cohort.

For the 2024 and 2025 locked-check procedures, apply this validation order to each
cohort independently:

1. establish the counted wagers under the already-selected frozen policy;
2. verify required settlement provenance for every counted wager;
3. if any counted wager has unresolved settlement provenance, emit the
   cohort-scoped `SETTLEMENT_PROVENANCE_FAILURE`, identify the affected cohort
   machine-readably, stop that cohort's ROI, bootstrap, and lower-bound
   computation, and do not continue to bootstrap-degeneracy evaluation as though
   settlement were valid;
4. only if settlement provenance passes, execute the frozen bootstrap; and
5. if any required bootstrap draw has zero counted wagers, emit
   `LOCKED_CHECK_BOOTSTRAP_DEGENERATE_FAIL_CLOSED` and identify the affected
   cohort machine-readably.

Settlement-provenance failure has terminal precedence over bootstrap degeneracy
for the same cohort because valid realized units are a prerequisite for ROI and
bootstrap calculation. An affected cohort does not invalidate the valid
2021-2023 selected policy or an unaffected other holdout cohort. This ordering
does not alter the development-level precedence rules.

For each applicable locked-check cohort independently, initialize one fresh
generator exactly as follows:
...
Use exactly 10,000 draws and the cohort's canonical ordered season-week cluster
frame. For draw indices `b = 0, 1, 2, ..., 9999`, with `n` observed clusters,
make exactly one call per draw:
...
Interpret the returned integers as zero-based indices into that cohort's
canonical ordered cluster frame. No other RNG-consuming call may occur between
initialization and completion of the required draw sequence. The 2024 and 2025
generators are independently freshly initialized, so results do not depend on
which cohort executes first.

For each cohort retain the NumPy version, canonical ordered cluster frame,
complete index representation, and SHA-256 of the draw evidence. Use the same
...
Missing prices must not be imputed. A missing valid designated-book reference
observation must emit `REFERENCE_MARKET_UNAVAILABLE`. A missing valid observation
from any member of a frozen consensus universe must emit
`REFERENCE_MARKET_INCOMPLETE`. Every pre-wager exclusion must retain a
...
calculation without imputing, excluding, redrawing, or substituting. Stop that
cohort before bootstrap-degeneracy evaluation; settlement-provenance failure has
terminal precedence because valid realized units are required for ROI and
bootstrap calculation. This does not retroactively invalidate the valid
development policy-selection result and does not automatically invalidate the
other holdout cohort when it is unaffected.
...
- eligible reference-book universe, containing at least two sportsbooks when a
  consensus method is selected;
...
- frozen reference-book universe and per-game reference coverage;
- reference no-vig probabilities;
- machine-readable `REFERENCE_MARKET_UNAVAILABLE` and
  `REFERENCE_MARKET_INCOMPLETE` exclusions;
- executable prices;
- model edge;
