# NFL Historical Market Research Preregistration 0.2.1

Protocol: `nfl_historical_market_research_0.2.1`
...
`nfl_historical_market_research_0.2.0`. The complete `0.1.0` and `0.2.0`
preregistrations remain immutable historical records in
...
outputs, outcomes, or derived betting performance. Version `0.2.1` closes the
counted-wager, ROI, clustered-bootstrap, numeric-reproducibility, settlement,
diagnostic-scope, and review-governance gaps identified during independent review
of candidate `0.2.0`. All compatible constraints from the predecessors remain
effective through the explicit rules below.
...
authoritative canonical scheduled kickoff:

```text
target_timestamp = canonical kickoff - 60 minutes
```

The deterministic selector is:

```text
selected_observation = latest eligible market observation whose observation
...
are independently validated and explicitly frozen in that amendment.
...
The policy-development cohort is 2021-2023 only. Sample eligibility is determined
on the observed development dataset before bootstrap resampling. A threshold is
sample-eligible only when it produces at least 100 counted development wagers.
...
may not be resampled independently. If any statistic required for the joint
simultaneous procedure is undefined, selection fails closed rather than
substituting another sampling or uncertainty method.

## Frozen bootstrap RNG and draw evidence
...
RNG contract, and retained-draw requirements separately to the single frozen
policy in each cohort. It must use the same deviation and nearest-rank
critical-value calculation specialized to a one-policy family and report the
resulting one-sided 95% ROI lower bound. This does not create a new selection
test or allow threshold revision.
...
deterministic, machine-readable reason together with source and mapping
provenance.
...
For postponed or rescheduled games, retain one canonical identity, reconcile the
provider event and observation to the authoritative canonical kickoff, and apply
`T-60` to that kickoff. Exclude the event before a wager exists if mapping or
timing lineage is ambiguous.
...
- verified provider/book settlement metadata;
- timestamp cadence and coverage quality;
- licensing;
- cost; and
- storage terms.

These parameters may be researched only with outcome-blind provider-feasibility
evidence. They must be resolved and frozen in a final pre-join protocol amendment
before historical odds are joined to model outputs, outcomes, or performance.
There is no separate open nearest-prior-tolerance parameter.
...
Any normative analysis specification referenced by this protocol is part of the
preregistration boundary. Before joined-performance code runs, each active
protocol and normative analysis specification must have an immutable
chain-of-custody record containing:
...
- active protocol version and hash; and
- active normative analysis-specification version and hash.

No joined-performance code may run unless those identities have first been
recorded in the analysis configuration and retained evidence. A normative
specification may not be modified after joined performance becomes visible while
retaining confirmatory status.
...
identities, input hashes, code and normative-specification identities, RNG and
draw evidence, decimal context, and validation result.
