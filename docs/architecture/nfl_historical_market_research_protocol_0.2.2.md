# NFL Historical Market Research Preregistration 0.2.2

Protocol: `nfl_historical_market_research_0.2.2`
...
`nfl_historical_market_research_0.2.1`. The complete `0.1.0`, `0.2.0`, and
`0.2.1` preregistrations remain immutable historical records in
...
outputs, outcomes, or derived betting performance. Version `0.2.2` narrowly
hardens historical kickoff and market-timestamp PIT authority, the active-bundle
one-way door, zero-denominator bootstrap handling, the conditional bootstrap
estimand, seed provenance, and threshold-nesting validation. It does not redesign
`0.2.1`. All compatible predecessor constraints remain effective through the
...
joint simultaneous statistic cannot be computed. Primary selection must then
fail closed. Do not drop the draw, redraw it, smooth the denominator, or
...
Future implementation must validate, over one identical market-eligible
population, the following nesting:
...
Any violation fails validation because it indicates eligibility, threshold, or
numeric-contract drift. This invariant is a validation rule, not a new policy-
selection rule.
