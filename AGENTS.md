...
- When authoritative sources disagree, do not choose silently. Prefer executable code, tests, and authorized effective-live evidence for current behavior, and identified immutable records for frozen research boundaries. Report the conflict and stop consequential work when it affects safety, authorization, or scope.
...
- Immediately before staging, committing, or pushing, verify the branch, HEAD, relevant remote ref, ahead/behind state, and all staged, modified, and untracked paths. Before pushing, verify the exact authorized commits, destination, and current remote state. Commit authorization is not push authorization.
...
- Before any historical market data are joined to model probabilities, outcomes, or derived performance, resolve the active chain-of-custody or freeze record and verify every declared downstream gate. Freeze and independently review required provider amendments and normative analysis specifications, record active artifact identities and hashes, and activate the active-bundle one-way door before the first joined-performance computation. A frozen base protocol alone does not authorize the join.
...
- Preserve canonical identity and source provenance. Treat persisted provider mappings as claims that must be revalidated against the authoritative identity evidence applicable to the workflow and its point-in-time boundary, including participants, orientation, event timing, source identity, and uniqueness constraints as appropriate. Reconcile automatically only when exactly one canonical identity is supported; fail closed on stale or conflicting mappings, participant or orientation conflicts, multiple plausible candidates, or insufficient point-in-time provenance. Preserve the original mapping and run evidence. Do not use an odds or market feed to invent canonical teams or games when the workflow requires existing canonical records.
...
- A production schema guard must run before provider access, run reservation, persistence, or other side effects. Define compatibility by explicitly verifying required migration identities and/or schema capabilities, not by assuming the latest migration. A maximum-version query such as `MAX(version)` is diagnostic context only and does not prove that required migrations exist. Test missing-required, minimum-compatible, and newer-compatible states where applicable.
...
- When an official point-in-time window expires, preserve the failed or partial run and its lineage as a historical gap. Do not reconstruct, backfill, or relabel an official result using later evidence, and do not overwrite retained incident evidence. Any separately authorized recovery must remain explicitly non-official unless the frozen workflow contract says otherwise.
...
- At meaningful high-impact milestones, obtain independent review before commit or production execution when the maintained workflow or user requires it. Preserve, as appropriate, the primary artifacts, complete diffs or raw exports, material raw validation output, source revision, exact path set, commands/results, redacted environment identity, manifests, byte sizes, and hashes. A prose or CLI summary alone is insufficient for meaningful high-impact review. Keep raw review packages outside the repository by default; intentionally maintained review, approval, or chain-of-custody records may be repository artifacts. Approval applies only to that reviewed revision/file set/artifact identity; substantive changes require re-review. Do not generate external evidence packages without authorization or a maintained runbook requirement.
